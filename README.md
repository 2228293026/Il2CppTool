...
- **工具**：核心浏览面板。可开任意多个「类标签页」，每个标签页：
  ...
  - **参数预设**：反复调同一个方法时，把当前参数**存为预设**，之后一键载入；预设随配置持久化，重启仍在
    > 预设**只保存输入的文本**。引用类型（对象）参数不保存——那个对象可能早已被 GC 回收，复原陈旧指针会直接 use-after-free。载入时对象参数会清空，需要重新选一次。
  - 按对象查看运行时对象图（`dataMap`，显示实例字段与类型层级）
  - 长按某对象/方法可把它在**新标签页**中打开继续分析；已打开的标签页会保留在 tab 栏里
  - **关注值（Watch）**：检视器里数值字段右边有个 `W`，点一下就把它钉进关注列表，之后**每 200ms 自动重读**，值变了会变绿。血量、弹药、分数、开关标志这类「盯着看」的值不用再一次次点 `Refresh`。
    > 关注项各自持有一个 GC 强根（否则对象被回收后就是野指针），列表是**全局的**——切到别的对象检视器也还在。
  - **冻结值**：关注列表每一行有「冻结」按钮，点了之后**每帧**把这个值写回去。
    > 没有它的话，改完的值经常撑不过一帧——游戏自己的 `Update`（`health -= damage`）或服务器同步下一帧就覆盖回去了，于是体验是「我明明改了，它弹回来了」却查不出原因。冻结中的值用**不同配色**显示：它一直不变，很容易被误读成「游戏卡住了」。对象被销毁、路径失效、或写不进去时**自动解冻**并记一条改动记录，不会继续对着野指针每帧写。
    > 只有**原始类型**（int / float / bool / 字符串引用）的叶子字段能进关注列表，所以每帧写回的是「一个已加 GC 根的对象上的一个原始类型字段」——不会写坏对象结构。写入走字段编辑那套严格解析（整串必须吃掉 + 范围检查）。
...
- **改动记录**：把「这个工具改动了目标游戏的什么」记成一份可导出的清单。改了十几个字段之后想不起来「当时把哪个设成了多少」时看这一页。
  - **可以撤销**：数值字段的改动记成 `旧值 -> 新值`，每一行右边有「恢复」按钮，一点就把字段退回去。对改内存的工具来说，「退回去」比「看」常用得多 —— 冻了个值玩够了、补丁打错了、想让游戏恢复正常，每一次都需要撤销，而不是「记得当时改成了多少」再手填。
    > 记录表**接管**根对象的 GC 句柄（否则退不回去），并且在条目被淘汰（超过 512 条）或点「清空」时**自己归还**。忘记归还的后果不是多用一点内存，而是那些对象**永远不会被回收** —— 「清空改动记录」会变成一个看不见的泄漏。
    > 撤销走的是和写入**完全同一套**解析（整串必须吃掉 + 范围检查），对象已经被 GC 回收的条目没有按钮。：把「**这个工具改动了目标游戏的什么**」记成一份可导出的清单 —— 改了哪些字段、设成了什么值、给哪个方法打了补丁、关注了哪些值。
    > 这些改动**只存在于游戏进程内存里**：游戏一关全部消失，工具自己也不留任何痕迹。改了十几个字段之后想不起来「当时把哪个设成了多少」时，看这一页。**只读**，不参与任何游戏逻辑，所以记录失败也不会影响工具本身。
- **自检**：把「哪些前置条件成立」一次性摊开 —— il2cpp API、程序集数量、**触摸字段偏移**、ESP 的相机/投影/包围盒、当前持有的 GC 根数量。
  > 这个工具里很多问题是**静默**的：ESP 拿不到相机就只是不画框，触摸偏移错了菜单就完全没反应——用户看不出是哪一环坏了。自检指出是哪一项失败，「日志」页给出原因。**排查问题请先看这一页。**
...
工具的行为**只能在真实游戏里确认**。仓库里有一份逐条可执行的清单：

👉 **[docs/真机验证清单.md](docs/真机验证清单.md)**

覆盖启动耗时、搜索、对象检视、方法调用、Patcher、Dumper、日志页、ESP、全局稳定性共 9 组。
每条都写了「操作 → 期望 → 失败说明」，并标注了哪些是**改动前会静默破坏游戏状态**的高危项
（例如「改数值字段后直接点确认」曾会把截断后的值写回游戏内存）。
出问题时请用「日志」页的「复制全部」把内容带出来。
