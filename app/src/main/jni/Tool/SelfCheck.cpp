...
std::vector<Result> Collect()
{
    ...
    // ---- 类筛选工作线程 ----
    //
    // 线程没起来的话**不会崩、也不会报错**，只是每次搜索都要在渲染线程
    // 同步跑完 —— 表现是「一搜就卡住」。而这件事只会在你真的去搜索的
    // 时候才被发现，而且很难联想到是线程没起来。
    {
        if (ClassesTab::FilterWorkerRunning())
        {
            out.push_back(Ok("类筛选线程", "在运行（搜索在后台线程做）"));
        }
        else
        {
            out.push_back(Warn("类筛选线程",
                              "未运行 —— 搜索会在渲染线程同步执行，界面可能卡顿"));
        }
    }
