...
namespace Tool
{
    ...
    // 关注值列表放在**这里**（工具页末尾），而不是对象检视器里。
    //
    // 放在检视器里的后果：ImGuiJson 只有在**有打开的对象 tab** 时才会被调用，
    // 而轮询又发生在 DrawWatches 里面。于是「把对象 tab 全关掉」这个
    // 完全正常的操作会让关注列表**整块消失，并且停止刷新** ——
    // 再打开某个对象时，值是旧的，然后突然跳一下。
    //
    // 清单 3.9b 写的是「切到别的对象的检视器 → 列表仍然在」，
    // 那种情况确实是好的；但只要**一个对象 tab 都不开**，就露馅了。
    // 关注是全局功能，就该挂在一直都在的页面上。
