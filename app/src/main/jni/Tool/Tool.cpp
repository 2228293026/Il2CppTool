...
namespace Tool
{
    ...
    // ---- 工具页底部的两块「跨界面」内容 ----
    //
    // 它们都**不能**挂在对象检视器（ImGuiJson）里：
    //
    // ImGuiJson 只有在**有打开的对象 tab** 时才会被调用，而：
    //
    //  1) 关注列表的轮询就发生在 DrawWatches 里 → 「把对象 tab 全关掉」
    //     这个完全正常的操作会让关注列表**整块消失，并且停止刷新**；
    //     再打开某个对象时值是旧的，然后突然跳一下。
    //     清单 3.9b 写的是「切到别的对象的检视器 → 列表仍然在」，
    //     那种情况确实是好的 —— 只有一个对象 tab 都不开时才露馅。
    //
    //  2) 写入失败提示的设置方（软键盘回调、枚举选择、**参数解析**）
    //     可能来自**任何**界面 —— 参数解析那条在 CallerView 的页签里，
    //     和 ImGuiJson 是两个完全不同的界面。结果是「参数 health 无法解析」
    //     这句话出现在对象检视器里。
    //     更糟的是过期清除也在同一个 if 里：没有对象 tab 打开时**永远不清**，
    //     一分钟前的错误会一直留着，等你下次随便打开一个检视器才冒出来，
    //     指着一个你根本没在看的字段。
    //
