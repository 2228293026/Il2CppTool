...
// 返回值：成功恢复的条数。
//
// out 参数的**确切含义**（第 103 轮订正过 —— 我第一版把两者的
// 说法写反了，那比不说更糟）：
//
//   skipped —— 本来就**不可恢复**的条数。两种情况：
//               已经逐条恢复过（UndoById 会把 handle 和 oldValue 清掉），
//               或者当初就没有记录原值。
//               **这不是错误**，也不代表对象出问题了。
//   failed  —— 尝试恢复但**失败了**的条数。对象已被 GC 回收、
//               路径失效、类型变了……这一类才意味着
//               「游戏现在**不是**原样」。
//
// 界面上必须把这两种分开说。把 failed 说成「跳过」会让用户以为
// 一切正常；把 skipped 说成「对象已失效」会让用户以为游戏崩了。
